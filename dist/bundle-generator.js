...
function generateDtsBundle(entries, options = {}) {
    ...
    return entries.map((entryConfig) => {
        ...
        const excludedTypePatterns = (outputOptions.excludedTypes || []).map((pattern) => {
        ...
        function isNameExcluded(name) {
            return excludedTypePatterns.some((regex) => regex.test(name));
        }
        function isIdentifierExcluded(identifier) {
            return isNameExcluded(identifier.text);
        }
        function isSymbolExcluded(symbol) {
            if (symbol === null) {
                return false;
            }
            const actualSymbol = (0, typescript_1.getActualSymbol)(symbol, typeChecker);
            return excludedSymbols.has(actualSymbol) || isNameExcluded(actualSymbol.getName());
        }
        function markSymbolExcluded(symbol) {
            if (symbol === null) {
                return;
            }
            excludedSymbols.add((0, typescript_1.getActualSymbol)(symbol, typeChecker));
        }
        function isEntityNameExcluded(entityName) {
            ...
            if (ts.isPropertyAccessExpression(entityName)) {
                return isEntityNameExcluded(entityName.expression) || isIdentifierExcluded(entityName.name);
            }
            return false;
        }
        const rootFileExports = (0, typescript_1.getExportsForSourceFile)(typeChecker, rootSourceFileSymbol).filter((exp) => {
        ...
        function handleExportDeclarationFromRootModule(exportDeclaration) {
            ...
            function handleReExportWithModuleSpecifier(reExportDeclaration) {
                const referencedModuleSymbol = (0, typescript_1.getNodeOwnSymbol)(reExportDeclaration.moduleSpecifier, typeChecker);
            ...
            if (exportDeclaration.exportClause !== undefined && ts.isNamedExports(exportDeclaration.exportClause)) {
                ...
                if (exportDeclaration.moduleSpecifier !== undefined) {
                    handleReExportWithModuleSpecifier(exportDeclaration);
                    return;
                }
            }
        }
        ...
        function isNodeUsed(node) {
            if ((0, typescript_1.isNodeNamedDeclaration)(node)) {
                ...
                if (nodeName !== undefined && ts.isIdentifier(nodeName) && isIdentifierExcluded(nodeName)) {
                    markSymbolExcluded((0, typescript_1.getNodeSymbol)(node, typeChecker));
                    return false;
                }
            }
